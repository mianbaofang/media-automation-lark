...
5. **dry-run 验证**：各场景脚本加 `--dry-run` 先空跑。
...
| **D 智能搜索采集→分类 Markdown** | `scripts/collector.py` | 按 query 调用可用搜索后端 → 抓正文 → 按分类存 Markdown → 可选归档飞书 |

各场景脚本共用 `scripts/common.py`（配置、密钥、LLM、lark-cli 封装、HTTP 重试、renhua 润色）。所有脚本均支持 `--dry-run`（不写飞书，仅生成本地备份/预览）与 `--yes`（跳过高风险确认），首次运行务必先 `--dry-run` 验证。
...
- 场景 D：`python3 scripts/collector.py --query "LLM 应用落地" --category-map "AI:大模型,LLM,Agent" --dry-run`
  - 先用 `--offline-demo` 跑通分类/写盘（不联网）；用 `--no-fetch` 只列 URL 不抓正文；用 `--backends anysearch` 限定后端。

### 场景 D 详解（自动搜索 + 分类 Markdown）

流程：用户给出检索词（CLI `--query` 或 `config.json` 的 `search.queries`）
→ `collector.py` 运行时检测可用搜索后端，跨后端收集候选 URL（去重）

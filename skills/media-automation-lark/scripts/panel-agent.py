...
def default_state_dir() -> Path:
    """Return the same per-user state directory used by gui-panel.py."""
    if os.name == "nt":
...
def configure_state_dir(value: str | os.PathLike[str] | None) -> Path:
    """Point launcher state files at one explicit directory."""
    global PANEL_DIR, PID_FILE, LOG_FILE, ERR_FILE, LAUNCHER_FILE
...
def start_windows_process(host: str, port: int, lang: str = "zh-CN") -> int:
        ...
        "\r\n".join(
            [
                ...
                (
                    ...
                    f'--host "{host}" --port "{port}" --state-dir "{PANEL_DIR}" --lang "{lang}" '
...
def start_posix_process(host: str, port: int, lang: str = "zh-CN") -> int:
    cmd = [
        sys.executable,
        str(ROOT / "scripts" / "gui-panel.py"),
        "--host",
        host,
        "--port",
        str(port),
...
def start(host: str, port: int, open_browser: bool, state_dir: str | os.PathLike[str] | None = None, lang: str = "zh-CN") -> dict:
    ...
    try:
        new_pid = start_windows_process(host, port, lang) if os.name == "nt" else start_posix_process(host, port, lang)
    except Exception as exc:  # noqa: BLE001
        return payload("error", host, port, None, "启动面板失败，请查看 panel.log。")
    ready = False
    for _ in range(20):
...
def stop(host: str, port: int) -> dict:
    pid = read_pid()
    active_pid = pid_for_port(port) or (pid if process_alive(pid) else None)
    if not active_pid:
        return payload("stopped", host, port, pid, "Panel was not running.")
    ok, message = terminate_pid(active_pid)
    if not ok:
        return payload("error", host, port, active_pid, "停止面板失败，请查看 panel.err.log。")
    try:
        PID_FILE.unlink()
    except OSError:
        pass
...
def main() -> None:
    ...
    parser.add_argument("--lang", choices=("zh-CN", "en"), default="zh-CN", help="面板语言")
    parser.add_argument("--open", action="store_true", help="Ask OS to open the panel URL")
    args = parser.parse_args()
    configure_state_dir(args.state_dir)

    if args.action == "start":
        result = start(args.host, args.port, args.open, lang=args.lang)

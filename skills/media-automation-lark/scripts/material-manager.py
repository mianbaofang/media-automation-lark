...
Markdown 后再做 AI 分析；MarkItDown 缺失时 PDF 回退 pypdf、其它格式跳过。
"""
...
def read_pdf(path: str):
    # 优先 MarkItDown：保留标题/表格/列表结构；不可用或失败再回退 pypdf
    if file2md.is_available():
        try:
            ...
            common.logger.warning("MarkItDown 转 PDF 失败，回退 pypdf: %s", e)

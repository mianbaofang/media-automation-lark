...
```powershell
python scripts/env-check.py --auto-install
Copy-Item -LiteralPath config.json.example -Destination config.json
...
```powershell
python scripts/collector.py --offline-demo --output-dir output_demo --no-archive --no-notify
python scripts/content-archiver.py --rss-url "https://example.com/feed.xml" --dry-run
python scripts/collector.py --query "LLM 应用落地" --rank-by hotness --dry-run
